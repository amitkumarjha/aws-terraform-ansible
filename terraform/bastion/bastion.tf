...
resource "aws_instance" "production-network-bastion" {
  ...
  subnet_id = "${data.terraform_remote_state.common.production-subnetwork-public.id}"
  vpc_security_group_ids = ["${data.terraform_remote_state.common.production-security-public.id}"]
  connection {
        user = "${var.ec2-user}"
